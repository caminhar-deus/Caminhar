...
export default function (data) {
  ...
  const createPayload = JSON.stringify({
    url: videoUrl,
    title: videoTitle,
    description: `Descrição do vídeo de teste ${uniqueId}.`,
  });
  ...
  const updatePayload = JSON.stringify({ title: updatedTitle, url: videoUrl });

  const updateRes = http.put(`${BASE_URL}/api/admin/videos/${videoId}`, updatePayload, {
    headers: authHeaders,
  ...
  // --- 3. DELETAR (DELETE) ---
  const deleteRes = http.del(`${BASE_URL}/api/admin/videos/${videoId}`, null, {
    headers: authHeaders,
