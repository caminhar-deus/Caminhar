import { query, transaction } from '../infra/db.js';
import { createRecord, updateRecords, deleteRecords, raw } from '../crud/crud.js';
 ...
 * @param {string} [orderBy='position ASC, created_at DESC'] - Cláusula ORDER BY personalizada.
 * @returns {Promise<Object>} Vídeos paginados com metadados.
 */
export async function getPaginatedVideos(page = 1, limit = 10, search = '', publishedOnly = false, orderBy = 'position ASC, created_at DESC') {
 ...
 * @returns {Promise<Object>} Vídeos paginados com metadados.
 */
export async function getPublicPaginatedVideos(page = 1, limit = 10, search = '', orderBy = 'position ASC, created_at DESC') {
  return getPaginatedVideos(page, limit, search, true, orderBy);
}
 ...
 */
export async function updateVideo(id, videoData, options = {}) {
  const videoDataFiltered = {};

  if (videoData.titulo !== undefined) videoDataFiltered.titulo = videoData.titulo;
  if (videoData.url_youtube !== undefined) videoDataFiltered.url_youtube = videoData.url_youtube;
  if (videoData.descricao !== undefined) videoDataFiltered.descricao = videoData.descricao;
  if (videoData.thumbnail !== undefined) videoDataFiltered.thumbnail = videoData.thumbnail;
  if (videoData.publicado !== undefined) videoDataFiltered.publicado = videoData.publicado;

  if (Object.keys(videoDataFiltered).length > 0) {
    videoDataFiltered.updated_at = raw('CURRENT_TIMESTAMP');
  }

  const [updatedRecord] = await updateRecords('videos', videoDataFiltered, { id }, options);
