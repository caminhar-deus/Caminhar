...
export async function getPaginatedVideos(page = 1, limit = 10, search = '') {
  ...
  if (search) {
    ...
    whereClause = `WHERE LOWER(titulo) LIKE $${searchParams.length + 1}`;
...
export async function getPublicPaginatedVideos(page = 1, limit = 10, search = '') {
  ...
  if (search) {
    const searchTerm = `%${search.toLowerCase()}%`;
    // FIX: Append search term with AND, not overwrite.
    whereClause += ` AND LOWER(titulo) LIKE $${searchParams.length + 1}`;
    searchParams.push(searchTerm);
  }
