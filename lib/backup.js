...
export async function restoreBackup(backupFilename) {
  console.warn('A restauração do PostgreSQL deve ser feita manualmente usando psql.');
  console.warn(`Exemplo: gunzip < ${backupFilename} | psql "${process.env.DATABASE_URL}"`);
  await logBackupOperation('RESTORE_INFO', `Tentativa de restauração manual para ${backupFilename}`);
  return false;
}
