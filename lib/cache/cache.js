import { getRedisInstance, redisGet, redisSet, redisDel, redisScan, redisIncr, redisExpire, redisFlushdb } from '../infra/redis.js';
...
// Não inicia em ambiente de teste para evitar que o setInterval mantenha
...
/**
 * Limpa todo o cache de aplicação em memória.
 * Útil em testes para evitar contaminação de estado entre cenários.
 */
export function clearAppMemoryCache() {
...
async function fetchAndCache(key, fetchFunction, ttlSeconds) {
  ...
  if (data) {
    // Serializa uma única vez e reusa para Redis e cache em memória
    ...
    // Salva string já serializada no cache em memória, evitando nova serialização
    const ttlMs = Math.floor(ttlSeconds) * 1000;
    appMemoryCache.set(key, {
      data: serialized,
      expiresAt: Date.now() + ttlMs,
...
export async function checkRateLimit(ip, endpoint, limit = 300, windowMs = 60000) {
  try {
    ...
    if (isRedisConnected()) {
      try {
        const redisKey = `rate_limit:${endpoint}:${ip}`;
    ...
    logger.error('Cache', 'Erro inesperado no checkRateLimit:', error.message);
    return false; // Nunca bloqueia por engano em caso de erro inesperado
  }
}