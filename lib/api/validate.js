...
export function validateBody(schema) {
  return (handler) => (req, res) => {
    ...
    try {
      ...
      if (error.issues) { // More robust check than instanceof
        return validationError(res, 'Erro de validação no corpo da requisição', formatZodErrors(error));
      }
      
      logger.error('Validate', 'Erro inesperado na validação do body:', error);
...
export function validateQuery(schema) {
  return (handler) => (req, res) => {
    try {
      ...
      if (error.issues) { // More robust check than instanceof
        return validationError(res, 'Erro ao validar parâmetros da URL', formatZodErrors(error));
      }
      
      logger.error('Validate', 'Erro inesperado na validação dos query params:', error);
...
export function validateParams(schema) {
  return (handler) => (req, res) => {
    try {
      // Extrai APENAS os parâmetros de rota dinâmica (definidos no schema) do req.query
      // O Next.js coloca tanto route params quanto query strings em req.query,
      // por isso precisamos selecionar apenas as chaves esperadas pelo schema.
      ...
      req.params = schema.parse(params);
      return handler(req, res);
    } catch (error) {
      if (error.issues) { // More robust check than instanceof
      ...
      logger.error('Validate', 'Erro inesperado na validação dos params:', error);
...
export function validateHeaders(schema) {
  return (handler) => (req, res) => {
    try {
      ...
      logger.error('Validate', 'Erro inesperado na validação dos headers:', error);
...
export function validateRequest(schemas = {}) {
  return (handler) => (req, res) => {
    ...
    if (schemas.body && ['POST', 'PUT', 'PATCH'].includes(req.method)) {
      ...
      if (!result.success) {
        allErrors.push(...formatZodErrors(result.error).map(e => ({ ...e, location: 'body' })));
      } else {
    ...
    if (schemas.query) {
      ...
      if (!result.success) {
        allErrors.push(...formatZodErrors(result.error).map(e => ({ ...e, location: 'query' })));
      } else {
        req.query = result.data;
      }
    }
    
    // Valida params (extrai apenas as chaves definidas no schema, evitando mistura com query strings)
    if (schemas.params) {
      const paramKeys = Object.keys(schemas.params.shape || {});
      const params = {};
      for (const key of paramKeys) {
        if (req.query[key] !== undefined) {
          params[key] = req.query[key];
      ...
      if (!result.success) {
        allErrors.push(...formatZodErrors(result.error).map(e => ({ ...e, location: 'params' })));
      } else {
