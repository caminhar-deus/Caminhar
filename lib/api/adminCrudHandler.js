...
export function createAdminHandler(config) {
  ...
  async function handler(req, res) {
    ...
    // ── 2. RBAC — administrador obrigatório ───────────────────────
    ...
    // ── 3. RBAC — verificação de permissão específica ─────────────
    if (permission) {
    ...
    // ── 5. Execução do handler específico ─────────────────────────
    ...
    try {
      ...
      // ── 6. Invalidação automática de cache em mutações bem-sucedidas ──
