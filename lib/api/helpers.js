...
export function detectSpoofedIP(req) {
  ...
  if (normalizedSocketIP && normalizedSocketIP !== forwardedIP) {
    // Caso 1: Socket é localhost (::1 ou 127.0.0.1)
    // Qualquer IP forwarded diferente do socket real é spoofing, pois
    // localhost não precisa de proxy e não deve ter X-Forwarded-For.
    // IPs privados também são rejeitados porque um atacante pode
    // usar ranges internos para tentar evadir a detecção.
