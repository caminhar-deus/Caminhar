...
export function getClientIP(req, options = {}) {
  ...
  // Último fallback: se chegou até aqui sem retorno, garante 127.0.0.1
  return '127.0.0.1';
}
 ...
 * @param {Object} req - Objeto de requisição Next.js/Express
 * @param {Object} [options]
 * @param {boolean} [options.strictMode=false] - Se true, detecta spoofing mesmo quando socket é localhost (usado em testes de segurança)
 ...
 *
 * @example
 * // Modo strict para testes de segurança
 * const { isSpoofed } = detectSpoofedIP(req, { strictMode: true });
 */
export function detectSpoofedIP(req, options = {}) {
  ...
  // Portanto, quando strictMode=false, sempre que o socket é localhost, NÃO bloqueamos o acesso.
  if (normalizedSocketIP === '127.0.0.1' && !strictMode) {
  ...
  if (normalizedSocketIP && normalizedSocketIP !== forwardedIP) {
    ...
    // Em strictMode=true, detecta spoofing mesmo com socket localhost
    if ((normalizedSocketIP === '127.0.0.1' || normalizedSocketIP === '::1') && !strictMode) {
      return { isSpoofed: false, socketIP: normalizedSocketIP, forwardedIP };
    }
