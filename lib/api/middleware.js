...
// Armazena referências dos timers para cleanup em testes
const activeTimers = [];

/**
 * Limpa todos os timers ativos (usado em testes)
 */
...
/**
 * Middleware de detecção de IP spoofing
 * Bloqueia requisições onde o header X-Forwarded-For não corresponde ao IP real do socket.
 * @param {Object} [options]
 * @param {boolean} [options.blockOnSpoof=true] - Se true, bloqueia a requisição com 403
 * @returns {Function} Middleware function
 */
export function withSpoofProtection(options = {}) {
  const { blockOnSpoof = true } = options;

  return (handler) => async (req, res) => {
    const { isSpoofed, socketIP, forwardedIP } = detectSpoofedIP(req);
    
    if (isSpoofed && blockOnSpoof) {
      console.warn(`[SpoofDetection] 🚫 IP spoofing detectado: socket=${socketIP}, forwarded=${forwardedIP}`);
      return forbidden(res, 'IP spoofing detectado. Requisição bloqueada.');
    }
