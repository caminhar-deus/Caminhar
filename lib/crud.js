...
const tableSchemas = {
  ...
  products: ['id', 'name', 'description', 'price', 'image_url', 'category', 'link', 'published', 'publicado', 'position', 'created_at', 'updated_at'],
};
...
function _filterAllowedFields(table, data) {
  ...
  if (!allowedFields) {
    ...
    logger.warn('CRUD', 'Tabela \'' + table + '\' não possui schema definido em crud.js. Pulando validação de campos.');
    return data;
  }
  ...
  for (const [field, value] of Object.entries(data)) {
    if (allowedFields.includes(field)) {
      ...
      logger.warn('CRUD', 'Campo \'' + field + '\' ignorado em ' + table + ': não pertence ao schema da tabela.');
    }
  }
  return filtered;
}
