...
function _filterAllowedFields(table, data) {
  ...
  if (!allowedFields) {
    ...
    console.warn(`Tabela '${table}' não possui schema definido em crud.js. Pulando validação de campos.`);
    return data;
  }
  ...
  for (const [field, value] of Object.entries(data)) {
    if (allowedFields.includes(field)) {
      ...
      console.warn(`Campo '${field}' ignorado em ${table}: não pertence ao schema da tabela.`);
    }
  }
  return filtered;
}
