import { redis, redisGet, redisSet, redisDel, redisScan, redisIncr, redisExpire, redisFlushdb, checkRedisHealth } from './redis.js';
...
cleanupTimer = setInterval(() => {
  ...
  if (localRateLimitMap.size > 5000) {
    ...
    if (removed > 0) {
      console.warn('[Cache] ⚠️ Rate Limit Map limite máximo: removidas ' + removed + ' entradas antigas');
    }
    ...
    if (localRateLimitMap.size > 5000) {
      ...
      console.warn('[Cache] ⚠️ Rate Limit Map: liberadas ' + toDelete.length + ' entradas excedentes (lazy eviction)');
    }
  }
  ...
  if (appMemoryCache.size > 2000) {
    ...
    if (removed > 0) {
      console.log('[Cache] 🧹 Cache app: ' + removed + ' entradas expiradas removidas (safety net)');
    }
  }
}, 60000);
...
export async function getOrSetCache(key, fetchFunction, ttlSeconds = DEFAULT_TTL) {
  ...
  if (redis) {
    ...
    try {
      ...
      console.error('[Cache] ❌ Erro de Cache Redis (GET) para chave ' + key + ':', error);
      metrics.redisErrors++;
      // Fallback para memória em caso de erro Redis
      cachedData = getAppMemoryCache(key);
      if (cachedData) {
        metrics.memoryHits++;
        return typeof cachedData === 'string' ? JSON.parse(cachedData) : cachedData;
      }
      // Se falhou tudo, busca do banco
  ...
  // 2. Tenta cache em memória (fallback se Redis não configurado ou cache miss no Redis)
  ...
  // 3. Cache Miss — busca do banco
  metrics.redisMisses++;
  if (!redis) metrics.memoryMisses++;
  return await fetchAndCache(key, fetchFunction, ttlSeconds);
}
...
export async function clearAllCache(options = {}) {
  ...
  try {
    ...
    console.error('[Cache] ❌ Erro ao executar FLUSHDB no Redis:', error);
    metrics.redisErrors++;
...
export async function checkRateLimit(ip, endpoint, limit = 300, windowMs = 60000) {
  ...
  if (redis) {
    try {
      ...
      console.warn('[Cache] ⚠️ Falha no Redis para ' + endpoint + ', ativando fallback em memória:', err.message);
