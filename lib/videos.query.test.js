...
import { getPaginatedVideos } from './videos.js';
import * as db from './db.js';

describe('getPaginatedVideos (Query SQL)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('deve gerar a query SQL com LIMIT e OFFSET corretos para a primeira página', async () => {
    // Mock dos retornos (getPaginatedVideos executa duas queries em paralelo via Promise.all)
    // Não podemos garantir a ordem exata de resolução, mas podemos inspecionar as chamadas.
    db.query
    ...
    await getPaginatedVideos(page, limit);

    // Encontra a chamada correspondente à query de seleção de dados
    const calls = db.query.mock.calls;
    const dataQueryCall = calls.find(call => call[0].includes('SELECT * FROM videos'));
  ...
  it('deve incluir cláusula WHERE e parâmetro de busca quando fornecido', async () => {
    ...
    const search = 'Culto';
    await getPaginatedVideos(1, 10, search);

    const calls = db.query.mock.calls;
    const dataQueryCall = calls.find(call => call[0].includes('SELECT * FROM videos'));
    const [text, values] = dataQueryCall;

    expect(text).toContain('WHERE LOWER(titulo) LIKE $3');
    expect(values[2]).toBe('%culto%');
  });
});