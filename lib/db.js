...
export async function getPaginatedMusicas(page = 1, limit = 10, search = '') {
  ...
  // Adiciona a cláusula WHERE apenas se houver um termo de busca válido
  if (search) {
    const searchTerm = `%${search.toLowerCase()}%`;
    const whereClause = ` WHERE (LOWER(titulo) LIKE $${placeholderIndex} OR LOWER(artista) LIKE $${placeholderIndex})`;
    text += whereClause;
    countText += ` WHERE (LOWER(titulo) LIKE $1 OR LOWER(artista) LIKE $1)`; // count query is simpler
    values.push(searchTerm);
    countValues.push(searchTerm);
    placeholderIndex++;
  }

  text += ` ORDER BY created_at DESC LIMIT $${placeholderIndex++} OFFSET $${placeholderIndex++}`;
