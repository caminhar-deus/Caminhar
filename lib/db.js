...
function createPool() {
  ...
  const newPool = new Pool({
    ...
    idleTimeoutMillis: 30000, // Tempo de timeout de conexões ociosas (reduzido de 60s para 30s)
    connectionTimeoutMillis: 5000, // Tempo de timeout de conexão (reduzido de 10s para 5s)
  });
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (attempt < maxAttempts && (
        ...
        logger.warn(
          'DB',
          `Tentativa ${attempt} de ${maxAttempts} falhou por timeout. Resetando pool e tentando novamente...`
        );
        if (!client) resetPool();
      ...
      if (process.env.LOG_LEVEL === 'debug') {
        ...
        logger.error('DB', 'Erro ao executar consulta SQL:', error.message);
      }
