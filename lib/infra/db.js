...
function createPool() {
  ...
  // Pré-aquece o pool com uma conexão inicial imediatamente na criação do pool,
  // para que a primeira conexão já esteja sendo estabelecida o quanto antes
  // (elimina o atraso artificial do setTimeout e cobre o primeiro request).
  // Não executa em ambiente de teste para evitar manter o event loop aberto.
  if (process.env.NODE_ENV && process.env.NODE_ENV !== 'test') {
    newPool.connect()
      .then(async (client) => {
        try {
