...
if (!JWT_SECRET) {
  ...
  // O fallback (chave derivada) é permitido apenas em desenvolvimento/teste.
  // Em qualquer outro ambiente (ex.: staging), a ausência de JWT_SECRET deve falhar.
  if (env !== 'development' && env !== 'test') {
    throw new Error('JWT_SECRET environment variable is required outside development/test environments');
  }
  // Gera chave determinística a partir do ambiente local
  const envHash = createHash('sha256')
    .update(process.cwd() + env + 'caminhar-dev-salt')
    .digest('hex');
...
export function verifyToken(token) {
  try {
    ...
    // Não expõe o motivo ao cliente (retorno null), mas registra em nível debug apenas
    // o tipo do erro — sem conteúdo do token — para não permitir flood de logs,
    // já que verifyToken é chamado em todas as requisições autenticadas.
    logger.debug('Auth', `Falha ao verificar token: ${err?.name || 'TokenError'}`);
    return null;
  }
}
 ...
 * 
 * O rate limiting (proteção contra brute force) é aplicado no middleware
 * proxy.js com a mesma chave 'api:auth:login', em ponto único — antigamente
 * a contagem era feita aqui e no proxy em duplicidade.
 * 
 * @param {string} username - Nome de usuário
 * @param {string} password - Senha do usuário
 * @param {string} ip - Endereço IP do cliente (para rate limiting)
 * @param {Object} [_options] - Opções adicionais (mantido para compatibilidade de assinatura com o consumidor)
 * @returns {Promise<Object>} Objeto com { user, token } em caso de sucesso,
 *                            ou { error: 'INVALID_CREDENTIALS', message } em caso de falha
 */
export async function authenticateAndGenerateToken(username, password, ip, _options = {}) {
  ...
  try {
    ...
    // Não bloqueia o login — o access token continua válido.
    // O refresh token não persistido nunca é devolvido ao cliente.
    refreshToken = null;
  }

  return { user, token, refreshToken, permissionsLoaded, error: null };
}
...
export function withAuth(handler) {
  return async (req, res) => {
    ...
    if (!token) {
      logger.warn('Auth', `Acesso negado: token não fornecido${req.url ? ` (${req.url})` : ''}`);
      return res.status(401).json({ message: 'Não autenticado' });
    }

    const decoded = verifyToken(token);
    if (!decoded) {
      logger.warn('Auth', `Acesso negado: token inválido ou expirado${req.url ? ` (${req.url})` : ''}`);
