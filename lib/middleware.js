/**
 * @deprecated Este arquivo está depreciado. Use `lib/api/middleware.js` no lugar.
 * 
 * Migração:
 ...
 * - `logger`                        → `withLogger()`
 * 
 * NOTA: `authenticatedApiMiddleware` e `externalAuthMiddleware` foram removidos.
 * Use `withAuth(handler)` de `lib/auth.js` ou `withAuth(options)` de `lib/api/middleware.js`.
 * 
 * Este arquivo será removido em uma versão futura após todos os consumidores serem migrados.
 */
 ...
 * @deprecated Use `composeMiddleware(withCors(), withErrorHandler(), handler)` de `lib/api/middleware.js`
 */
 ...
 * @deprecated Use `withRateLimit()` de `lib/api/middleware.js`
 */
 ...
 * @deprecated Use `withErrorHandler()` de `lib/api/middleware.js`
 */
