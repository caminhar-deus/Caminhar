...
import { getPaginatedPosts } from '../../../../../lib/posts.js';
import * as db from '../../../../../lib/db.js';

describe('getPaginatedPosts (Query SQL)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('deve gerar a query SQL com LIMIT e OFFSET corretos para a primeira página', async () => {
    // Mock dos retornos (getPaginatedPosts executa duas queries em paralelo via Promise.all)
    ...
    await getPaginatedPosts(page, limit);

    // Encontra a chamada correspondente à query de seleção de dados
    const calls = db.query.mock.calls;
    const dataQueryCall = calls.find(call => call[0].includes('SELECT * FROM posts'));
  ...
  it('deve incluir cláusula WHERE e parâmetro de busca quando fornecido', async () => {
    ...
    const search = 'Reflexão';
    await getPaginatedPosts(1, 10, search);

    const calls = db.query.mock.calls;
    const dataQueryCall = calls.find(call => call[0].includes('SELECT * FROM posts'));
    const [text, values] = dataQueryCall;

    expect(text).toContain('WHERE LOWER(title) LIKE $3');
    expect(values[2]).toBe('%reflexão%');
  });
});