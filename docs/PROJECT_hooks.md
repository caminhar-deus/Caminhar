...
- **`handleDelete(id)`:** Se `onConfirmDelete` for fornecido, aguarda a Promise resolver (`await onConfirmDelete(id)` — recebe o `id` do item). Se resolver com `true`, prossegue com o `DELETE`. Caso contrário, usa `window.confirm` como fallback. Envia `DELETE` com `Content-Type: application/json` e `{ id }` no corpo JSON (necessário para o servidor parsear o `id` e efetivar a exclusão), atualiza a lista via `refetch`.
