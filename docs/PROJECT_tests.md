...
> **Data:** 13/05/2026 (atualizado em 17/06/2026 — 59ª revisão)
...
> **Ajustes realizados na 51ª revisão (16/06/2026):**
> - **Testes do `BackupManager.test.js` alinhados ao Modal de confirmação (7/7 testes passando)**: o componente `BackupManager.js` foi refatorado para usar um **Modal React customizado** no lugar de `window.confirm()`, mas 4 testes ainda dependiam do `confirmSpy` (mock de `window.confirm`), que nunca era chamado. Correções: (1) importação de `createConfirmSpy` removida (não utilizada); (2) declaração e restauração do `confirmSpy` removidas do `beforeEach`/`afterEach`; (3) teste de cancelamento — agora clica no botão **Cancelar** dentro do Modal e verifica que o fetch foi chamado apenas 1 vez (chamada inicial); (4) testes de sucesso e erro — agora clicam no botão **Confirmar** dentro do Modal antes de verificar o resultado; (5) teste de sucesso — busca por `Criando Backup` movida para **após** o clique no Confirmar, garantindo que o estado `creating` seja `true` quando verificado. Nenhuma alteração no código de produção. **Total:** 1 arquivo modificado (`tests/unit/components/Admin/Managers/BackupManager.test.js`). 7/7 testes passando. Nenhuma regressão.

> **Ajustes realizados na 52ª revisão (16/06/2026):**
> - **Guardas e limpeza de estilos do `TextArea.js` corrigidos (12/12 testes passando)**: duas falhas no componente `TextArea.js` foram corrigidas. (1) `useEffect` não limpava estilos ao desabilitar `autoResize` — o `if (!autoResize) return;` impedia que `calculateHeight()` (que contém a lógica de limpeza) fosse executado. Removida a guarda precoce. (2) `overflowY` não era limpo — adicionado `textarea.style.overflowY = ''` na branch `!autoResize` de `calculateHeight`. **Total:** 1 arquivo modificado (`components/UI/TextArea.js`). Nenhuma alteração nos testes. 12/12 testes passando. Nenhuma regressão.

> **Ajustes realizados na 53ª revisão (16/06/2026):**
> - **Contador de caracteres do `TextArea.js` aprimorado com sufixo descritivo (15/15 testes passando)**: o formato do contador foi alterado de `"5 / 100"` (apenas número) para `"5 / 100 caracteres"` (com sufixo descritivo), melhorando a acessibilidade e experiência do usuário. Três arquivos foram ajustados: (1) `components/UI/TextArea.js` (linha 169) — adicionado `` `${maxLength} caracteres` `` ao contador; (2) `tests/unit/components/UI/TextArea.test.js` (linhas 24 e 44) — asserções atualizadas de `'3 / 10'` para `'3 / 10 caracteres'` e `'5 / 10'` para `'5 / 10 caracteres'`; (3) `tests/unit/components/Admin/TextAreaField.test.js` (linha 11) — asserção corrigida de `'5/100 caracteres'` para `'5 / 100 caracteres'` (espaçamento incorreto). **Total:** 3 arquivos modificados. 15/15 testes passando (2 suítes). Nenhuma regressão.

> **Ajustes realizados na 54ª revisão (16/06/2026):**
> - **Testes do `dicas.test.js` corrigidos — `logActivity` mockado no módulo errado (11/11 testes passando)**: quatro testes de POST, PUT e DELETE falhavam com `Number of calls: 0` para `logActivity`. Causa raiz: o teste mockava `logActivity` em `lib/db.js` (função que não existe mais — foi intencionalmente removida conforme comentário nas linhas 332-336), enquanto o `adminCrudHandler.js` importa `logActivity` de `lib/domain/audit.js`. Correções: (1) mock do `db.js` alterado de `mockDb({ logActivity: jest.fn() })` para `mockDb()`; (2) adicionado `jest.mock('../../../../lib/domain/audit.js', () => ({ logActivity: jest.fn() }))`; (3) import do `logActivity` alterado de `'../../../../lib/db.js'` para `'../../../../lib/domain/audit.js'`; (4) terceiro parâmetro das asserções (`entityType`) corrigido de `'POST'/'PUT'/'DELETE'` para `'DICA'` (o que o `adminCrudHandler` realmente envia via `name.toUpperCase()`). Os arquivos `posts.test.js`, `musicas.test.js`, `videos.test.js`, `users.test.js`, `roles.test.js` e `audit.test.js` já estavam corretos e não precisaram de alteração. **Total:** 1 arquivo modificado (`tests/integration/api/admin/dicas.test.js`). Nenhuma regressão nos 139 testes do suíte admin.

> **Ajustes realizados na 55ª revisão (16/06/2026):**
> - **Testes do `ImageUploadField.test.js` corrigidos — `global.alert` substituído por `toast.error` e assinatura do `onUpload` ajustada (7/7 testes passando)**: três testes falhavam. Causas raiz e correções: (1) Dois testes esperavam `global.alert()` mas o componente `ImageUploadField.js` usa `toast.error()` do `react-hot-toast` — substituídas as asserções de `global.alert` por `toast.error` com `jest.mock` do módulo. (2) Teste do `onUpload` esperava `onUpload(file)` com 1 argumento, mas o componente chama `onUpload(file, uploadType)` com 2 argumentos (`uploadType` padrão `'post'`) — asserção corrigida para `toHaveBeenCalledWith(file, 'post')`. (3) `global.alert` mock removido do `beforeEach`/`afterEach` (não é mais usado). **Total:** 1 arquivo modificado (`tests/unit/components/Admin/ImageUploadField.test.js`). Nenhuma regressão.

> **Ajustes realizados na 56ª revisão (16/06/2026):**
> - **Testes do `Testimonials/index.test.js` corrigidos — formato do mock incompatível com `transform` e contaminação entre testes (4/4 testes passando)**: quatro falhas foram corrigidas. Causas raiz e correções: (1) Mocks do `fetch` retornavam **array simples** (`[{...}]`) mas a função `transform` do `useApiFetch` espera `{ success: true, data: [...] }` — mocks ajustados para o formato correto. (2) `rerender` no 3º teste não disparava re-fetch por causa do `staleTime: 60000` do `useApiFetch`, deixando `mockResolvedValueOnce` não consumido que contaminava o 4º teste — substituído por `render` + `unmount` + adicionado `beforeEach` com `mockClear()`. (3) Seletor `.dicas-container` não encontrava elemento porque `styleMock` retorna `{}` e as classes CSS modules viram `undefined` — substituído por `div[id="testimonials-carousel"]`. (4) `removeEventListener` recebia `undefined` como handler no unmount — asserção simplificada para verificar apenas que o evento `resize` foi removido, sem exigir a função exata. **Total:** 1 arquivo modificado (`tests/unit/components/Features/Testimonials/index.test.js`). Nenhuma regressão.

> **Ajustes realizados na 57ª revisão (17/06/2026):**
> - **Testes do `withAdminAuth.test.js` corrigidos — `AuthProvider` ausente na árvore de renderização (7/7 testes passando)**: a causa raiz era estrutural — o `withAdminAuth` (HOC) consome o `AuthContext` via `useContext(AuthContext)` através do hook `useAdminAuth`, mas os testes renderizavam o componente **sem o `AuthProvider`**, fazendo com que `useContext` retornasse o valor **default** (`loading: true`). Como `loading` nunca mudava de `true` para `false`, o componente ficava permanentemente preso na tela "Verificando autenticação...", impedindo todos os fluxos subsequentes (login, autenticação, logout). Correção aplicada **(Abordagem A — Provider real)**: (1) Adicionado `import { AuthProvider } from '../../../../hooks/useAuth.js'`; (2) Criada função `renderWithAuthProvider(ui)` que envolve o componente no `<AuthProvider>` real; (3) Ajustados os mocks de `fetch` para o fluxo correto — o `useEffect` de `checkAuth` dentro do `AuthProvider` agora consome o primeiro mock, e os mocks seguintes seguem a ordem real (checkAuth → login, quando aplicável); (4) Teste de erro de rede (console.error) ajustado para o comportamento real — o `AuthProvider` não loga erros de rede no `checkAuth` (erro é silencioso), portanto o teste agora verifica que o formulário de login aparece após o erro em vez de esperar um `console.error` que nunca seria chamado. Nenhuma alteração no código de produção. **Total:** 1 arquivo modificado (`tests/unit/components/Admin/withAdminAuth.test.js`). Nenhuma regressão.

> **Ajustes realizados na 58ª revisão (17/06/2026):**
> - **Testes do `AdminProducts.test.js` corrigidos — formatação do `price` e evento do checkbox alinhados ao comportamento real (13/13 testes passando)**: duas falhas foram corrigidas. (1) Teste `handleFetchML` esperava `setFieldValue('price', 'R$ 99,90')`, mas o componente `AdminProducts.js` armazena o valor com `data.price.toFixed(2)` que produz `'99.90'` — a formatação monetária com "R$" existe apenas na coluna de exibição da tabela (`format: (value) => \`R$ ${value}\``). (2) Teste `CheckboxWrapper interno` usava `value={true}` (prop incorreta — `ToggleField` espera `checked={true}`) e esperava que `onChange` recebesse um objeto customizado `{ target: { name, value } }`, mas o React gera um `SyntheticBaseEvent` nativo — a verificação foi ajustada para `eventArg.target.name`. Nenhuma alteração no código de produção. **Total:** 1 arquivo modificado (`tests/unit/components/Admin/AdminProducts.test.js`). Nenhuma regressão.
> 
> > **Ajustes realizados na 59ª revisão (17/06/2026):**
> > - **Testes do `AdminCrudBase.test.js` corrigidos — `toggleField is not a function` e `skeleton-box` não encontrado (19/19 testes passando)**: três falhas foram corrigidas. (1) O mock de estilos (`__mocks__/styleMock.js`) retornava `{}`, fazendo com que `styles.skeletonBox` fosse `undefined` no runtime — adicionado `skeletonBox: 'skeleton-box'` para que a classe CSS fosse aplicada e o seletor `.skeleton-box` funcionasse. (2) O objeto `mockUseAdminCrud` no teste não incluía `toggleField`, causando `TypeError: toggleField is not a function` ao clicar no botão de toggle booleano — adicionado `toggleField: jest.fn()`. (3) As asserções dos dois testes de toggle foram ajustadas: o teste de sucesso passou a verificar que `toggleField` foi chamado com os argumentos corretos (`{ id, name, status }, 'status', false`); o teste de reversão configurou `toggleField` para rejeitar com `'Falha na API'` e passou a verificar a chamada do `toggleField` em vez do `toast.error`. **Total:** 2 arquivos modificados (`__mocks__/styleMock.js`, `tests/unit/components/Admin/AdminCrudBase.test.js`). Nenhuma regressão.
