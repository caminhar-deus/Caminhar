...
### 8.13 Expansão de cobertura do teste de `RateLimitViewer` (36,09% → ≥ 90%)

**Arquivo:** `tests/unit/components/Admin/Tools/RateLimitViewer.test.js`

**Descrição:** O teste foi reescrito de 1 para 21 casos. O mock de `fetch` passou a despachar por URL seguindo as formas reais da API `pages/api/admin/rate-limit.js` (`/api/admin/rate-limit` → `[{ ip, count, ttl }]`, `?type=whitelist` → `['ip', ...]`, `?type=audit` → `{ logs, pagination }`, POST/DELETE → `{ message }`). Cenários cobertos: render inicial com verificação das credenciais das chamadas paralelas; listagem de IPs bloqueados com detalhes; respostas não-array tratadas como listas vazias; desbloqueio de IP (sucesso e erro); whitelist com itens; adição à whitelist (sucesso, erro e IP vazio sem chamada de API); remoção da whitelist (sucesso e erro); logs de auditoria com busca e paginação (incluindo botões desabilitados nos extremos); erro de auditoria via `console.error`; estado de erro com reintento ("Tentar Novamente"); sessão expirada (401) em bloqueados, whitelist e auditoria; falha de rede; refresh manual com promise controlada ("Atualizando..."); auto-refresh de 15s com `jest.useFakeTimers`. Para o caso de 401 na auditoria, o componente passou a tratar a resposta na própria rota (`fetchAuditLogs`), acompanhando o mesmo padrão de `fetchData`.
...
### 8.28 Eliminação dos warnings de `act` e do ruído de navegação do jsdom no teste de `RateLimitViewer` (microtasks do auto-refresh aguardadas e `console.error` suprimido nos 401)

**Arquivo:** `tests/unit/components/Admin/Tools/RateLimitViewer.test.js`

**Descrição:** A suíte emitia 4 `console.error` `An update to RateLimitViewer inside a test was not wrapped in act(...)` — com stack em `RateLimitViewer.js:47`, `:48`, `:52` e `:53` (`setBlockedIps`, `setWhitelist`, `setLoading(false)` e `setRefreshing(false)`) — e 2 erros `Not implemented: navigation (except hash changes)` do jsdom a partir de `RateLimitViewer.js:37` (`window.location.reload()` dos testes de 401 de bloqueados e de whitelist). Os warnings de `act` eram exibidos dentro do bloco de `AdminAudit.test.js` no `logs/coverage-output.log` e os erros de navegação dentro do bloco de `AdminCrudBase.test.js`, embora nenhuma das duas suítes os produzisse: executadas isoladamente, ambas terminam com 0 ocorrências. Causa raiz: o caso `deve auto-refrescar os dados cada 15 segundos` avançava os timers em um `act` síncrono (`act(() => { jest.advanceTimersByTime(15000); })`), que dispara o `setInterval` do componente sem drenar a cadeia de promises de `fetchData` (mock de `fetch`, `await` dos mocks assíncronos e `.json()`); as atualizações de estado resolviam depois do fim do caso e do `cleanup()` do `afterEach`, já fora da associação com o arquivo de origem. O avanço dos timers passou a ser aguardado com `await act(async () => { jest.advanceTimersByTime(15000); })`, preservando as asserções de `fetch` do caso (2 chamadas iniciais e 4 após o auto-refresh). Nos 2 testes de 401 foi adicionado `const consoleErrorSpy = suppressConsoleError();` no início e `consoleErrorSpy?.mockRestore();` no fim, o mesmo padrão já usado nos demais testes de 401 e de erro de auditoria do arquivo — a interceptação do reload no jsdom atual não é possível (`jest.spyOn(window.location, 'reload')` responde `Cannot assign to read only property 'reload'` e `Object.defineProperty(window, 'location', …)` responde `Cannot redefine property: location`). Nenhuma alteração foi feita em código de produção.

**Resultado:** As 4 ocorrências de `An update to RateLimitViewer inside a test was not wrapped in act(...)` e as 2 de `Not implemented: navigation` deixaram de ser emitidas, tanto na suíte isolada (`grep -c 'not wrapped in act'` e `grep -c 'Not implemented: navigation'` = 0, antes 4 e 2) quanto no run completo em `logs/coverage-output.log`, com a suíte mantendo 21/21 testes aprovados. Cobertura global: 95,38% statements/lines, 88,15% branches e 94,55% functions, com `npm run test:coverage:log` retornando status 0 e 1229 testes aprovados (181 suites).
