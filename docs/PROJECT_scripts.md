...
| `clean-orphaned-images.js` | Remove imagens órfãs de teste (`post-image-*`, `hero-image-*`) em `public/uploads/` não referenciadas no banco (`posts.image_url`, `settings.value`). Usa pool direto com dotenv. |
