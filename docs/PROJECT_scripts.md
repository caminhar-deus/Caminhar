...
| `warm-routes.js` | **Pré-aquecimento de rotas (241 linhas).** Força compilação das rotas dinâmicas `/blog/[slug]` do Next.js/Turbopack (contorna bug "PageNotFoundError/ENOENT"). 5 fases: páginas estáticas, rotas de dados SSR, rotas SSR data, HTML completas, verificação final. Flags: `--slugs=`, `--base-url=`, `--retries=`. |
