...
> **Data da análise:** 23/09/2026 (atualização integral — processo sequencial arquivo por arquivo)
> **Objetivo:** Documentar todos os arquivos da pasta `/pages`, descrevendo localização exata, arquivos acionados/relacionados, propósito e funcionalidades de cada um.
...
3. [Páginas Raiz (`/pages`)](#1-páginas-raiz)
4. [API Pública (`/pages/api`)](#2-api-pública)
5. [API Admin (`/pages/api/admin`)](#3-api-admin)
6. [API Autenticação (`/pages/api/auth`)](#4-api-autenticação)
7. [Helper de API (`/pages/api/helper`)](#5-helper-de-api)
8. [Blog (`/pages/blog`)](#6-blog)
9. [Estilos (`/pages/styles`)](#7-estilos)
...
```text
/pages
├── _app.js                     # App wrapper global (providers, Toaster, router events)
...
│   ├── musicas.js              # Músicas públicas (paginado, busca, ordenação)
│   ├── videos.js               # Vídeos públicos (paginado, busca, ordenação)
│   ├── dicas.js                # Dicas públicas (paginado)
│   ├── products.js             # Produtos: CRUD público/autenticado
│   ├── settings.js             # Configurações do sistema (GET/POST/PUT)
│   ├── status.js               # Health check e diagnóstico
...
│   │   ├── check.js            # Verificação de token JWT
│   │   └── refresh.js          # Renovação de access token
│   ├── admin/
...
│   │   ├── roles.js            # Gestão de papéis e permissões
│   │   ├── audit.js            # Consulta de logs de auditoria
│   │   ├── stats.js            # Estatísticas do dashboard
│   │   ├── integrity.js        # Diagnóstico de integridade (5 dimensões)
│   │   ├── backups.js          # Gestão de backups do banco
│   │   ├── cache.js            # Gestão e métricas do cache
│   │   ├── rate-limit.js       # Gestão de rate limiting (Redis)
...
├── blog/
│   ├── index.js                # Listagem de posts (SSR)
│   ├── [slug].js               # Detalhe de post (SSR)
│   └── Blog.module.css         # CSS Module do blog
└── styles/
...
> **Nota:** o CRUD administrativo de produtos não existe como rota separada — é gerenciado pelo endpoint público `/pages/api/products.js` em modo autenticado (sem `?public=true`).
...
- **Arquivos acionados/relacionados:**
  - `pages/styles/globals.css` — estilos globais importados na inicialização.
  - `hooks/index.js` (via `../hooks`) — fornece `PerformanceProvider` e `usePerformance`.
  - `react-hot-toast` (dependência externa) — componente `<Toaster>`.
  - `next/router` — eventos de rota (`routeChangeComplete`).
  - Envolve todas as páginas da aplicação (toda a árvore renderizada passa por ele).
...
- **Arquivos acionados/relacionados:**
  - `lib/seo/config.js` — objeto `siteConfig` (idioma `pt-BR`, URL canônica, nomes da aplicação, `msapplication-tooltip`).
  - `components/Performance/CriticalCSS.js` — `extractCriticalCSS()` (o CSS bruto vive em `components/Performance/styles/criticalCSSRaw.js`).
  - `public/favicon.ico` — favicon referenciado no `<Head>`.
...
- **Funcionalidades:**
  ...
  - **Content Security Policy (CSP)** restritiva: `default-src 'self'`, scripts de YouTube/Spotify (com `'unsafe-eval' 'unsafe-inline'`, necessários ao Next.js), estilos de Google Fonts, imagens de `data:`/`https:`/`blob:`, frames de YouTube/Spotify, `object-src 'none'`, `upgrade-insecure-requests`.
  - **Permissions Policy** restritiva (geolocation, câmera, microfone, pagamento, USB, accelerometer, magnetometer bloqueados; `gyroscope=(self)`).
  - **Meta tags PWA**: `theme-color` adaptável (light `#2c3e50` / dark `#1a252f`), `apple-mobile-web-app-capable`, `apple-mobile-web-app-title`, `msapplication-*` (tooltip, starturl, navbutton-color).
...
- **Arquivos acionados/relacionados:**
  - `pages/styles/Home.module.css` — CSS Module da página.
  - `pages/api/settings.js` — `fetch('/api/settings')` para título e subtítulo dinâmicos.
  - `pages/api/placeholder-image.js` — URL estável da imagem hero (`/api/placeholder-image`).
  - `components/Features/ContentTabs` e `components/Features/Testimonials` — componentes renderizados abaixo do hero.
  - `public/favicon.ico` — ícone declarado no `<Head>`.
...
- **Arquivos acionados/relacionados:**
  - `components/Admin/styles/login.module.css`, `tabs.module.css`, `form.module.css`, `misc.module.css` — CSS Modules mapeados manualmente em um objeto `styles` unificado (evita conflito de classes homônimas entre módulos).
  - `components/Admin/AdminDashboard`, `AdminPosts`, `AdminMusicas`, `AdminVideos`, `AdminProducts`, `AdminDicas`, `AdminUsers`, `AdminAudit` — componentes de gestão por aba.
  - `components/Admin/Tools/IntegrityCheck`, `Tools/RateLimitViewer` e `Managers/BackupManager`, `Managers/CacheManager` — sub-abas de Segurança.
  - `pages/api/auth/login.js`, `auth/check.js`, `auth/logout.js` — autenticação.
  - `pages/api/settings.js` — leitura e gravação de título/subtítulo (GET e PUT).
  - `pages/api/upload-image.js` — upload da imagem principal (`uploadType=setting_home_image`).
  - `react-easy-crop` (Cropper) e `react-hot-toast` — dependências externas.
- **Propósito:** Painel administrativo completo para gestão de todo o conteúdo do site (780 linhas, é o maior arquivo da pasta).
- **Funcionalidades:**
  - **Autenticação:** login via `POST /api/auth/login` com `credentials: 'include'`, verificação de sessão via `GET /api/auth/check` (com `AbortController` para cancelamento), redirect pós-login para `/admin` e logout que limpa a sessão e recarrega a página.
...
- **Arquivos acionados/relacionados:**
  - `components/UI` (índice) — `Button`, `Input`, `TextArea`, `Select`, `Card`, `Modal`, `Spinner`, `Badge`, `Alert`, `Toast`.
  - `components/Layout` (índice) — `Container`, `Grid` (inclui `Grid.Responsive`), `Stack`, `Sidebar`.
  - `pages/styles/DesignSystem.module.css` — CSS Module da página.
...
- **Arquivos acionados/relacionados:**
  - Importado por `pages/api/dicas.js` (`paginate`, `buildPaginationMeta`, `paginatedResponse`) e `pages/api/products.js` (`paginate`).
  - Não importa nenhum arquivo — é um módulo puro, sem dependências.
- **Propósito:** Helper reutilizável de paginação (não é endpoint HTTP).
- **Funcionalidades:**
  - `paginate(rawPage, rawLimit, maxLimit=100)` — parseia e valida parâmetros, retorna `{ page, limit, offset }`; lança `INVALID_PAGINATION_PARAMS` se inválidos (page/limit < 1 ou limit > maxLimit; defaults: page 1, limit 10).
...
- **Arquivos acionados/relacionados:**
  - `lib/infra/db.js` — `query()` (PostgreSQL).
  - `lib/cache/cache.js` — `getOrSetCache` e `checkRateLimit`.
  - `pages/api/helper/pagination.js` — `paginate`, `buildPaginationMeta`, `paginatedResponse`.
  - `lib/api/helpers.js` — `getClientIP`.
...
- **Funcionalidades:**
  ...
  - Query com `SELECT COUNT(*)` + `SELECT ... WHERE published = true ORDER BY id ASC LIMIT/OFFSET` (parametrizado) em `Promise.all`.
...
- **Arquivos acionados/relacionados:**
  - `lib/domain/musicas.js` — `getPaginatedMusicas()` (busca paginada).
  - `lib/cache/cache.js` — `getOrSetCache` e `checkRateLimit`.
  - `lib/api/helpers.js` — `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
  - `zod` (dependência externa) — validação de query params.
- **Propósito:** Endpoint público de listagem de músicas com paginação, busca e ordenação.
- **Funcionalidades:**
  - Método GET apenas (405 com header `Allow` para outros).
  - Validação de query params via **Zod** (`z.coerce.number().int().positive()` para page/limit, `search` máx. 200 chars, `sort` enum `default|recent|alpha`), com erros de campo retornados em `errors` (400).
...
- **Arquivos acionados/relacionados:**
  - `lib/domain/posts.js` — `getRecentPosts()` (GET) e `createPost()` (POST).
  - `lib/cache/cache.js` — `getOrSetCache`, `checkRateLimit`, `invalidateCache`.
  - `lib/auth/auth.js` — `withAuth` (protege o POST).
  - `lib/api/helpers.js` — `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
  - `zod` (dependência externa) — `postCreateSchema`.
...
- **Arquivos acionados/relacionados:**
  - `lib/domain/videos.js` — `getPublicPaginatedVideos()`.
  - `lib/cache/cache.js` — `getOrSetCache`, `checkRateLimit`.
  - `lib/api/helpers.js` — `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
- **Propósito:** Endpoint público de listagem de vídeos com paginação, busca e ordenação.
- **Funcionalidades:**
  - Método GET apenas (405 sem header `Allow`).
...
- **Arquivos acionados/relacionados:**
  - `lib/domain/products.js` — `getPaginatedProducts`, `getAllProducts`, `createProduct`, `updateProduct`, `deleteProduct`.
  - `lib/domain/audit.js` — `logActivity` (auditoria nas mutações).
  - `lib/auth/auth.js` — `getAuthToken`, `verifyToken` (middleware `requireAuth` inline).
  - `lib/cache/cache.js` — `getOrSetCache`, `checkRateLimit`, `invalidateCache`.
  - `pages/api/helper/pagination.js` — `paginate`.
  - `lib/api/helpers.js` — `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
...
- **Funcionalidades:**
  ...
  - **GET** (autenticado): listagem via `getAllProducts(page, limit)`.
  - **POST/PUT/DELETE** (autenticado): cria, atualiza e remove via camada de domínio, com rate limiting em mutações (`api:admin:products`, 30 req/min), invalidação de cache (`products:*`) e `logActivity()` para auditoria.
...
- **Arquivos acionados/relacionados:**
  - `lib/domain/settings.js` — `getSettings`, `getSetting`, `updateSetting`, `getAllSettingsRaw`.
  - `lib/auth/auth.js` — `withAuth`, `getAuthToken`, `verifyToken`.
  - `lib/cache/cache.js` — `getOrSetCache`, `invalidateCache`, `checkRateLimit`.
  - `lib/api/helpers.js` — `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
  - `zod` (dependência externa) — validação.
- **Propósito:** Endpoint unificado de configurações do sistema.
- **Funcionalidades:**
  - **GET** (público, sem `?key`): retorna configurações via `getSettings()`/`getAllSettingsRaw()` com cache `settings:all` (TTL 2h; `settings:v1:all` no modo v1), Cache-Control `public, s-maxage=120, stale-while-revalidate=600`, e rate limiting `fire-and-forget` (`api:public:settings`, 600 req/min — `checkRateLimit(...).catch(() => {})` para falhas de rate limit nunca abortarem a resposta).
  - **GET** (autenticado, com `?key`): verificação manual de token (`getAuthToken` + `verifyToken`), restrição a roles `admin`/`editor`, cache `settings:${key}` (TTL 2h), fallback para valores padrão hardcoded (`site_name`, `site_description`, `posts_per_page`, `videos_per_page`, `musicas_per_page`) e 404 se a chave não existir.
...
- **Arquivos acionados/relacionados:**
  - `lib/infra/db.js` — `query()` (teste `SELECT 1` de conexão).
...
- **Funcionalidades:**
  ...
  - **GET** (padrão): retorna diagnóstico completo — versão da API, ambiente, timestamp, status do banco PostgreSQL (teste `SELECT 1`), e métricas do sistema (Node.js, plataforma, uptime, memória RSS/heap — comentário do código indica uso em monitoramento via k6).
...
- **Arquivos acionados/relacionados:**
  - `formidable`, `sharp`, `crypto`, `fs`, `path` (dependências/módulos) — parse multipart, validação de conteúdo e geração de nome seguro.
...
- **Arquivos acionados/relacionados:**
  - `lib/auth/auth.js` — `withAuth`.
  - `lib/infra/db.js` — `query()` (DELETE em `posts`).
  - `lib/infra/logger.js` — `logger`.
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler` (factory que envolve todos os handlers).
  - `lib/domain/posts.js` — `getPaginatedPosts`, `createPost`, `updatePost`, `deletePost`.
  - `lib/crud/crud.js` — `updateRecords` (reordenação em massa).
  - `lib/infra/db.js` — `query` (busca do título para log de auditoria no DELETE).
  - `zod` (dependência externa) — schemas `postCreateSchema`, `postUpdateDataSchema`, `reorderSchema`.
...
- **Arquivos acionados/relacionados:**
  ...
  - `lib/domain/musicas.js` — `getPaginatedMusicas`, `createMusica`, `updateMusica`, `deleteMusica`.
  - `lib/crud/crud.js` — `updateRecords` (reordenação).
  - `zod` (dependência externa) — `musicaSchema`, `reorderSchema`.
...
- **Funcionalidades:**
  ...
  - POST com validação Zod (`musicaSchema`) — título, artista e `url_spotify` obrigatórios; validação de URL do Spotify via `isValidSpotifyUrl()` (contém `spotify.com` ou `spotify:`); `publicado` default `false`.
  ...
  - Config: `rateLimit: 300/min`, `cacheKeys: 'musicas:*'` (sem `permission` explícita na config).
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/domain/videos.js` — `getPaginatedVideos`, `createVideo`, `updateVideo`, `deleteVideo`, `reorderVideos`.
  - `zod` (dependência externa) — `videoSchema` e `getValidationMessage`.
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/infra/db.js` — `query()` (SQL direto na tabela `dicas`).
  - `lib/cache/cache.js` — `invalidateCache('dicas:public:*')`.
  - `zod` (dependência externa) — `dicaSchema`, `dicaUpdateSchema`.
...
- **Funcionalidades:**
  ...
  - Config: `rateLimit: 30/min` (sem `permission` explícita na config).
...
- **Arquivos acionados/relacionados:**
  ...
  - `lib/crud/crud.js` — `createRecord`, `updateRecords`, `deleteRecords`.
  - `lib/auth/auth.js` — `hashPassword`.
  - `zod` (dependência externa) — `userCreateSchema`, `userUpdateSchema`.
- **Propósito:** CRUD administrativo de usuários (124 linhas).
- **Funcionalidades:**
  - GET com paginação (limite padrão 100) e busca por username (`LIKE` com `lower()`), **nunca retorna senhas** (seleciona apenas `id, username, role, created_at, last_login_at`).
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/infra/db.js` — `query()` (tabela `activity_logs`).
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/infra/db.js` — `query()` (contagens em `posts`, `musicas`, `videos`, `products`, `users`, `dicas`).
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `scripts/backup.js` — `createBackup()` (importado sem extensão `.js` explícita).
  - `fs` e `path` (módulos nativos) — leitura de `data/backups/`.
- **Propósito:** Gerenciamento de backups do banco de dados.
- **Funcionalidades:**
  - GET: lista arquivos de backup em `data/backups` (.sql/.gz/.enc) ordenados por data (mais recente primeiro) com nome, data e tamanho; retorna `{ latest, backups }`.
  - POST: cria backup via `createBackup()` de `scripts/backup` e registra auditoria.
  - Config: `rateLimit: 10/min` (sem `permission` explícita na config).
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/cache/cache.js` — `clearAllCache`, `getCacheMetrics`.
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `lib/infra/redis.js` — `redisScan` (SCAN paginado centralizado, mesma primitiva usada por `invalidateCache`).
  - `@upstash/redis` (dependência externa) — cliente Redis direto (pipeline, sadd/srem, lpush/ltrim/lrange).
  - `lib/infra/logger.js` — `logger`.
  - `zod` (dependência externa) — `ipSchema`.
...
- **Arquivos acionados/relacionados:**
  - `lib/api/adminCrudHandler.js` — `createAdminHandler`.
  - `zod` (dependência externa) — `urlSchema`.
  - API externa: `https://www.youtube.com/oembed` (oEmbed público).
- **Propósito:** Busca metadados de vídeo do YouTube.
- **Funcionalidade:** usa a API **oEmbed** pública (`youtube.com/oembed?url=...&format=json`) e retorna `{ title }`; registra auditoria. Timeout de 8s via `fetchWithTimeout()` com `AbortController`.
...
- **Arquivos acionados/relacionados:**
  ...
  - `zod` (dependência externa) — `urlSchema`.
  - `lib/infra/logger.js` — `logger`.
  - APIs externas: `open.spotify.com/oembed`, `open.spotify.com/embed/track/{id}` e a página pública do Spotify.
- **Propósito:** Busca metadados de música do Spotify.
- **Funcionalidades:** estratégias em cascata (timeout de 8s cada):
...
- **Arquivos acionados/relacionados:**
  ...
  - `zod` (dependência externa) — `urlSchema`.
  - `lib/infra/logger.js` — `logger`.
  - APIs externas: `api.mercadolibre.com/items/{id}`, `/items/{id}/description`, `/products/{id}` e a página pública do Mercado Livre.
- **Propósito:** Busca dados de produto do Mercado Livre para importação facilitada.
- **Funcionalidades:** decodifica a URL, extrai códigos `MLB` (regex `MLB[-_]?\d+`, remove duplicatas), prioriza `item_id` explícito (se presente) e tenta (timeout de 8s cada):
...
- **Arquivos acionados/relacionados:**
  - `lib/auth/auth.js` — `authenticateAndGenerateToken`, `setAuthCookie`, `setRefreshTokenCookie`.
  - `lib/api/helpers.js` — `detectSpoofedIP`, `getClientIP`.
  - `lib/infra/logger.js` — `logger`.
...
- **Funcionalidades:**
  ...
  - Rate limiting por IP (5 tentativas/60s) aplicado dentro de `authenticateAndGenerateToken()`.
  ...
  - Modo API externa (`?response=body`): retorna `{ token, token_type, expires_in, refresh_token, refresh_token_stored, refresh_token_expires_in, user }` no corpo. Quando o refresh token não foi persistido, `refresh_token` vem como `null`/ausente e `refresh_token_stored` como `false`.
...
- **Funcionalidades:**
  - GET apenas.
  - **Não usa `withAuth`** — valida manualmente com `getAuthToken(req)` (prioriza header `Authorization: Bearer` com fallback para cookie `token`) e `verifyToken(token)`.
...
- **Arquivos acionados/relacionados:**
  - `lib/auth/auth.js` — `refreshAccessToken`, `setAuthCookie`, `setRefreshTokenCookie`, `getRefreshTokenCookie`.
  - `lib/infra/logger.js` — `logger`.
...
- **Arquivos acionados/relacionados:**
  - `lib/auth/auth.js` — `setAuthCookie`, `setRefreshTokenCookie`, `getRefreshTokenCookie`, `revokeRefreshToken`.
...
- **Arquivos acionados/relacionados:**
  - `pages/blog/Blog.module.css` — CSS Module compartilhado do blog.
  - `components/Features/Blog/PostCard` — card de cada post.
  - `lib/infra/db.js` — `query()` (query SSR direta ao banco).
  - `next/head` e `next/link` — SEO e navegação.
...
- **Arquivos acionados/relacionados:**
  ...
  - `lib/infra/db.js` — `query()` (query SSR direta ao banco).
  - `next/head` e `next/link` — SEO e navegação.
  - `public/default-og-image.jpg` — imagem OG padrão quando o post não tem imagem.
...
- **Arquivos acionados/relacionados:**
  - Importado por `pages/blog/index.js` e `pages/blog/[slug].js`.
  - Consome CSS Custom Properties de `pages/styles/variables.css` (via `globals.css`).
- **Propósito:** Estilos CSS Module compartilhados entre as páginas do blog.
- **Estilos definidos:** `.container` (min-height 100vh, background `--color-bg-secondary`), `.header` (texto centralizado), `.title` (2.5rem), `.subtitle` (1.1rem), `.grid` (grid responsivo `minmax(300px, 1fr)` com gap 2rem, max-width 1200px) e media query para 768px.
...
- **Arquivos acionados/relacionados:**
  - Importado por `pages/styles/globals.css` via `@import` (único ponto de entrada — nenhum outro arquivo o importa diretamente).
  - Consumido indiretamente por toda a aplicação através das CSS Custom Properties.
- **Propósito:** **348 CSS Custom Properties** no `:root` (em 386 linhas) — a fonte única dos Design Tokens.
- **Categorias:** Primary/Secondary/Neutral/Feedback/Semantic/State/Spiritual Colors, Spacing e Spacing Semântico, Typography, Borders, Shadows (inclui `--shadow-glow` duplicada nas linhas 278 e 290), Breakpoints, Animation, Opacity e Z-Index.
- **Nota:** o comentário de cabeçalho do arquivo ainda diz "Gerado automaticamente a partir de /pages/styles/tokens/", mas essa pasta foi removida — o arquivo é hoje a fonte estática única (ver nota da seção).

### `/pages/styles/globals.css`

- **Caminho:** `/pages/styles/globals.css`
- **Arquivos acionados/relacionados:**
  - Importa `pages/styles/variables.css` via `@import`.
  - Importado por `pages/_app.js` (ponto de entrada dos estilos globais da aplicação).
  - `body.modal-open` é gerenciado pelo componente `Modal` (`components/UI`).
...
- **Arquivos acionados/relacionados:**
  - Importado por `pages/index.js` (home).
  - Consome CSS Custom Properties de `variables.css`.
...
- **Arquivos acionados/relacionados:**
  - Importado por `pages/design-system.js`.
  - Consome CSS Custom Properties de `variables.css`.
