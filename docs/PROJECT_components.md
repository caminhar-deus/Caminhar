...
**Funcionalidades:** Tabela configurável, formulário dinâmico, Drag & Drop com reversão automática em caso de falha, busca server-side, exportação CSV, toggle de status com rollback, paginação, modo somente leitura, skeletons, validação Zod, modal de confirmação assíncrono para exclusão via componente `Modal` da UI.
